...
public:
	...
	int inlierNum;
	bool doTrim;
    bool doKD; //disables the DT
    
    long long count_outer;
    long long count_inner_ub;
    long long count_inner_lb;
...
private:
	...
	float InnerBnB(float* maxRotDisL, TRANSNODE* nodeTransOut, long long& count, float ourMaxRotDis);
	float OuterBnB();
	void Initialize();
	void Clear();
