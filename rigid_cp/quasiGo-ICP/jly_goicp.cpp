...
GoICP::GoICP()
{
	...
	doTrim = true;
    doKD = false; //default value, can change in main
}
  ...
  int i;
	...
	for(i = 0; i < Nd; i++)
	{
		...
		if(!doTrim)
		{
            ...
            {
                dis = kd.Distance(pDataTempICP[i].x, pDataTempICP[i].y, pDataTempICP[i].z);
            }
			error += dis*dis;
		}
		else
		{   
            ...
            {
                minDis[i] = kd.Distance(pDataTempICP[i].x, pDataTempICP[i].y, pDataTempICP[i].z);
            }			
		}
	}
...
void GoICP::Initialize()
{
    ...
    //NADAV TEMP VAR
    localEnergy = (float*)malloc(sizeof(float)*Nd);
...
float GoICP::InnerBnB(float* maxRotDisL, TRANSNODE* nodeTransOut, long long& count, float ourMaxRotDis)
{
	int i, j;
	...
	//optErrorT = optError;
    optErrorT = optError+1.1*ourMaxRotDis;   // Investigating translation nodes that are sub-optimal overall is redundant-if they will give high lb anyway

	// Push top-level translation node into the priority queue
	queueTrans.push(initNodeTrans);
        
    //NADAV: precompute sum of norms
	...
	while(1)
	{
		...
		for(j = 0; j < 8; j++)
		{
			...
			for(i = 0; i < Nd; i++)
			{
                ...
                {
                    minDis[i] = kd.Distance(pDataTemp[i].x + transX, pDataTemp[i].y + transY, pDataTemp[i].z + transZ);
                }
			}
            ...
            //NADAV: compute energy for fixed R_c, t_c
            ub=0;
            for(i = 0; i < inlierNum; i++)
			{
				ub += minDis[i]*minDis[i];
               ...
               translationDelta=Nd*maxTransDis*maxTransDis;
                lb=ub-translationDelta;
                if (lb<0)
                    lb=0;
...
float GoICP::OuterBnB()
{
    ...
    //NADAV: my vars
    float M,N,ourMaxRotDis,temp;
	clock_t clockBeginICP;
	priority_queue<ROTNODE> queueRot;
    
    //NADAV: precompute sum of squared norm of data
    N=0;
    for(i = 0; i < Nd; i++)
    {
       N+= normData[i]*normData[i];   //REMINDER: normData[i] = sqrt(pData[i].x*pData[i].x + pData[i].y*pData[i].y + pData[i].z*pData[i].z);
    }
	...
	{   
        ...
        {
            minDis[i] = kd.Distance(pData[i].x, pData[i].y, pData[i].z);
        }
	}
	if(doTrim)
	{
	...
	//long long count_outer = 0;
    //long long count_inner_ub = 0;
    //long long count_inner_lb = 0;
    count_outer = 0;
    count_inner_ub = 0;
    count_inner_lb = 0;
	while(1)
	{
		...
		if(count_outer>0 && count_outer%300 == 0)
			printf("LB=%f  L=%d  (%d %d %d)\n",nodeRotParent.lb,nodeRotParent.l,count_outer,count_inner_ub,count_inner_lb);
		count_outer ++;
		//printf("%d %d %d\n",count_outer,count_inner_ub,count_inner_lb);
		...
		for(j = 0; j < 8; j++)
		{
		  // Calculate the smallest rotation across each dimension
            ...
            //count_inner_ub = 0;
            M=N+sqrt(N)*sqrt(optError);  //norm Data is sum of norms of x, optError is best ub found so far
            ourMaxRotDis=SQRT3*nodeRot.w/2;
            temp=exp(ourMaxRotDis)-1-ourMaxRotDis;
            ourMaxRotDis=2*M*temp;
            
			ub = InnerBnB(NULL /*Rotation Uncertainty Radius*/, &nodeTrans, count_inner_ub,ourMaxRotDis); //first and last input not needed probably
            ...
            //NADAV: for us lower bound is just a shift of the upper bound
            //lb = InnerBnB(maxRotDis[nodeRot.l], NULL /*Translation Node*/, count_inner_lb,SQRT3*nodeRot.w/2);
            
            lb=ub-ourMaxRotDis;
            if (lb<0)
                lb=0;
            //NADAV: for debugging(if(count_outer>0 && count_outer%300 == 0)
            //    printf("N=%f M=%f temp=%f ourMaxRotDis=%f ub=%f lb=%f optError=%f \n",N,M,temp,ourMaxRotDis,ub,lb,optError);
