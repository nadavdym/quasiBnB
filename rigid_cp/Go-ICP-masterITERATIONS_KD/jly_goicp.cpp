...
float GoICP::InnerBnB(float* maxRotDisL, TRANSNODE* nodeTransOut, long long& count)
{
	int i, j;
	...
	while(1)
	{
		...
		for(j = 0; j < 8; j++)
		{
		  // Calculate the smallest rotation across each dimension
            ...
            //count_inner_ub = 0;
			ub = InnerBnB(NULL /*Rotation Uncertainty Radius*/, &nodeTrans, count_inner_ub);
            ...
            //count_inner_lb = 0;
			lb = InnerBnB(maxRotDis[nodeRot.l], NULL /*Translation Node*/, count_inner_lb);
